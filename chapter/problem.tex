...
\choicelow{$Q\subseteq P$}{$Q\subset P$}{$P=Q$}{$P\subset Q$}

\item 设 \(A=\{a,\{a\}\}\),下列命题错误的是(\quad).
\choicelow{\(\{a\}\in P(A)\)}{\(\{\{a\}\}\in P(A)\)}{\(\{a\}\subseteq P(A)\)}{\(\{\{a\}\}\subseteq P(A)\)}

\item 在 $0(\ \ )\varnothing$ 之间写上正确的符号.
\choicelow{$\subseteq$}{$\in$}{$=$}{$\notin$}

\item 若 $A-B=\Phi$,则下列哪个结论不可能正确(\quad).
\choicelow{$A\subset B$}{$A=\Phi$}{$B\subset A$}{$B=\Phi$}

\item 判断下列哪个命题为真(\quad).
\choicemid{空集只是非空集合的子集}{若 $A$ 的一个元素属于 $B$,则 $A=B$}{$A-B=B-A \Rightarrow A=B$}{空集是任何集合的真子集}
...
\choicelow{满射不是单射}{双射}{单射不是满射}{不是单射也不是满射}

\item 下面函数(\quad)是单射而非满射.
\choicehigh{$f:\mathbb{R}\to\mathbb{R},\quad f(x)=2x+1$}{$f:\mathbb{R}\to\mathbb{Z},\quad f(x)=\lfloor x\rfloor$,$\lfloor x\rfloor$ 表示不大于 $x$ 的最大整数}{$f:\mathbb{R}\to\mathbb{R},\quad f(x)=-x^2+2x-1$}{$f:\mathbb{Z}^{+}\to\mathbb{R},\quad f(x)=\ln x$}
...
则 $R$ 具有(\qquad)性质.
\choicelow{自反性}{反自反性、反对称性、传递性}{反自反性、反对称性}{自反性、对称性、传递性}

\item 设 $|A|=n$,则 $A$ 上有(\quad)二元关系.
\choicelow{$2^n$}{$2^{n^2}$}{$n^2$}{$n^n$}

\item  设 \(A=\{\varnothing,\{1\},\{1,3\},\{1,2,3\}\}\),则 \(A\) 上包含关系“\(\subseteq\)”的哈斯图为(\quad).
\choicemid{\begin{tikzpicture}[scale=0.9, every node/.style={font=\small}]
		\node (t) at (0,3) {\(\{1,2,3\}\)};
		\node (m2) at (0,2) {\(\{1,3\}\)};
		\node (m1) at (0,1) {\(\{1\}\)};
		\node (b) at (0,0) {\(\varnothing\)};
		\draw (t)--(m2)--(m1)--(b);
	\end{tikzpicture}}{\begin{tikzpicture}[scale=0.9, every node/.style={font=\small}]
		\node (t) at (0,2) {\(\{1,2,3\}\)};
		\node (l) at (-1.2,1) {\(\{1\}\)};
		\node (r) at (1.2,1) {\(\{1,3\}\)};
		\node (b) at (0,0) {\(\varnothing\)};
...
\choicemid{\( f=\left\{\langle x_1,x_2\rangle \mid x_1,x_2\in\mathbb{R},\,x_1=x_2^2\right\}\)}{\( f=\left\{\langle x_1,x_2\rangle \mid (x_1,x_2\in\mathbb{N} )\land (x_1+x_2=10)\right\}\)}{\( f=\left\{\langle x,\lvert x\rvert\rangle \mid x\in\mathbb{R}\right\}\)}{\( f=\left\{\langle x_1,x_2\rangle \mid x_1,x_2\in\mathbb{N},\,x_2\text{为小于}x_1\text{的素数的个数}\right\}\)}
...
则 $R$ 具有(\quad) 的性质.
\choicelow{反自反、反对称、传递}{自反、对称、传递}{什么性质也没有}{自反、对称、反对称、传递}


\item 设 $A=\{1,2,3\}$,则 $A$ 上有(\quad)个二元关系.

\choicelow{$2^{3^2}$}{$2^3$}{$2^{2^3}$}{$3^2$}


\item 设 \(R,\,S\) 是集合 \(A\) 上的关系,则下列(\quad)断言是正确的.
\choicemid{若 \(R,\,S\) 传递的,则 \(R\circ S\) 是传递的}{若 \(R,\,S\) 反对称的,则 \(R\circ S\) 是反对称的}{若 \(R,\,S\) 对称的,则 \(R\circ S\) 是对称的}{\(R,\,S\) 自反的,则 \(R\circ S\) 是自反的}
...
\choicehigh{\begin{tikzpicture}[scale=0.9, every node/.style={font=\small}]
		\node (1) at (0,0) {1};
		\node (2) at (-0.9,1.0) {2};
		\node (4) at (0.9,1.0) {4};
		\node (3) at (0,-1.2) {3};
...
\choicemid{无,4,5,2,3,4,5,1,1,4,4}{无,4,2,3,4,5,1,1,4,4}{无,4,2,3,4,1,1,4,无}{无,4,2,3,4,1,1,4,4}
...
\begin{enumerate}
	\item 永真式的否定是(\quad).
	\choicelow{(B)--(D)均有可能}{永真式}{可满足式}{永假式}
	
	\item 命题逻辑演绎的 CP 规则为(\quad).
	\choicemid{在推演过程中可随便使用前提}{在推演过程中可随便使用前面演绎出的某些公式的逻辑结果}{设 $\Phi(A)$ 是含公式 $A$ 的命题公式,$B \Leftrightarrow A$,则可用 $B$ 替换 $\Phi(A)$ 中的 $A$}{如果要演绎出的公式为 $B \to C$ 形式,那么将 $B$ 作为前提,设法演绎出 $C$}
	
	\item $\neg (p \wedge q) \to r$ 的主析取范式中含极小项的个数为(\quad).
	\choicelow{3}{0}{5}{2}
	
	\item 下列语句是命题的有(\quad).
	\choicemid{我正在说谎}{$x+y>0$}{$xy>0$ 且仅当 $x$ 和 $y$ 都大于 0}{明年中秋节的晚上是晴天}
	
	\item 下列符号串是合式公式的有(\quad).
	\choicelow{$P \Leftrightarrow Q$}{$P \Rightarrow P \vee Q$}{$\neg (P \Leftrightarrow Q)$}{$(\neg P \vee Q)\wedge (P \vee \neg Q)$}
	
	\item 下列哪些公式为永真蕴含式？(\quad)
	\choicelow{$\neg Q \Rightarrow P \to Q$}{$\neg Q \Rightarrow Q \to P$}{$\neg P \wedge (P \vee Q) \Rightarrow \neg P$}{$P \Rightarrow P \to Q$}
	
	\item 全体小项合取式为(\quad).
	\choicelow{$A,B,C$ 都有可能}{矛盾式}{永真式}{可满足式}
	
	\item 下列公式是重言式的有(\quad).
	\choicelow{$\neg (Q \to P)\wedge P$}{$(P \wedge Q)\to Q$}{$\neg (P \Leftrightarrow Q)$}{$(P \to Q)\Leftrightarrow P$}
	
	\item 下列等价式成立的有(\quad).
	\choicelow{$P \wedge (P \to Q)\Leftrightarrow Q$}{$P \to (Q \to R)\Leftrightarrow (P \wedge Q)\to R$}{$P \vee (P \wedge R)\Leftrightarrow R$}{$P \to Q \Leftrightarrow \neg Q \to \neg P$}
	
	\item $(P \to Q)\to R$ 的合取范式为(\quad).
	\choicemid{$(P \vee Q \vee R)\wedge (P \vee \neg Q \vee R)\wedge (P \vee \neg Q \vee R)\wedge (\neg P \vee \neg Q \vee R)$}{$(P \vee R)\wedge (\neg Q \vee R)$}{$(P \wedge \neg Q)\vee R$}{$(P \wedge \neg Q \wedge R)\vee (P \wedge \neg Q \wedge \neg R)\vee (P \wedge Q \wedge R)\vee (P \wedge \neg Q \wedge R)\vee (\neg P \wedge Q \wedge R)\vee (\neg P \wedge \neg Q \wedge R)$}
	
	\item 下列等价式成立的有(\quad).
	\choicelow{$P \vee (P \wedge R)\Leftrightarrow R$}{$P \to Q \Leftrightarrow \neg Q \to \neg P$}{$P \to (Q \to R)\Leftrightarrow (P \wedge Q)\to R$}{$P \wedge (P \to Q)\Leftrightarrow Q$}
	
	\item 下列各命题中真值为真的命题有(\quad).
	\choicelow{$2+2=4$ 当且仅当 $3$ 是奇数}{$2+2\neq 4$ 当且仅当 $3$ 是奇数}{$2+2\neq 4$ 当且仅当 $3$ 不是奇数}{$2+2=4$ 当且仅当 $3$ 不是奇数}
	
	\item 下列语句是命题的有(\quad).
	\choicemid{$xy>0$ 且仅当 $x$ 和 $y$ 都大于 0}{我正在说谎}{明年中秋节的晚上是晴天}{$x+y>0$}
	
	\item 下列哪些是简单命题？
	\choicelow{中国有四大发明}{3 是素数或者 4 是素数}{刘红与赵欣是同学}{你去图书馆吗}
	
	\item 下列公式中哪些是永真式？(\quad)
	\choicelow{$(\neg P \wedge Q)\to (Q\to \neg R)$}{$(P \wedge Q)\to P$}{$P\to (P\vee Q)$}{$P\to (Q\to Q)$}
	
	\item $A,B$ 为二合式公式,且 $A \Leftrightarrow B$,则(\quad).
	\begin{enumerate}[label=\Alph*., leftmargin=2.5em, itemsep=0pt, topsep=0pt, partopsep=0pt, parsep=0pt]
...
\end{enumerate}
	...
	\item 下列问题成立的有(\quad).
	\choicelow{若 $A \wedge C \Leftrightarrow B \wedge C$,则 $A \Leftrightarrow B$}{若 $A \vee C \Leftrightarrow B \vee C$,则 $A \Leftrightarrow B$}{若 $\neg A \Leftrightarrow \neg B$,则 $A \Leftrightarrow B$}{若 $A \Leftrightarrow B$,则 $\neg A \Leftrightarrow \neg B$}
...
\begin{enumerate}
	...
	\choicelow{无法判定}{可满足式}{\(1\)}{\(0\)}
	
	\item 命题“有的人喜欢所有的花”的逻辑符号化为(\quad).
	
	设 \(D\)：全总个体域,\(F(x)\)：\(x\) 是花,\(M(x)\)：\(x\) 是人,\(H(x,y)\)：\(x\) 喜欢 \(y\).
	\choicemid{\(\forall x\bigl(M(x)\to \forall y(F(y)\to H(x,y))\bigr)\)}{\(\exists x\bigl(M(x)\to \forall y(F(y)\to H(x,y))\bigr)\)}{\(\forall x\bigl(M(x)\land \forall y(F(y)\to H(x,y))\bigr)\)}{\(\exists x\bigl(M(x)\land \forall y(F(y)\to H(x,y))\bigr)\)}
	
	\item 下列等价关系正确的是(\quad).
	\choicemid{\(\neg \forall x(P(x)\vee Q(x))\Leftrightarrow \neg \forall xP(x)\vee \neg \forall xQ(x)\)}{\(\forall x(P(x)\to Q)\Leftrightarrow \forall xP(x)\to Q\)}{\(\exists x(P(x)\vee Q(x))\Leftrightarrow \exists xP(x)\vee \exists xQ(x)\)}{\(\exists x(P(x)\to Q)\Leftrightarrow \exists xP(x)\to Q\)}
	
	\item “人总是要死的”谓词公式表示为(\quad).
	
	(论域为全总个体域)\(M(x)\)：\(x\) 是人;\(\operatorname{Mortal}(x)\)：\(x\) 是要死的.
	\choicemid{\(\exists x\bigl(M(x)\land \operatorname{Mortal}(x)\bigr)\)}{\(\forall x\bigl(M(x)\to \operatorname{Mortal}(x)\bigr)\)}{\(M(x)\land \operatorname{Mortal}(x)\)}{\(M(x)\to \operatorname{Mortal}(x)\)}
	...
	\end{tabular}
	\choicelow{\textcircled{4}}{\textcircled{3}}{无}{\textcircled{2}}
...
\end{enumerate}
	...
	\choicemid{\(\forall x\forall y(P(x,u)\vee Q(u,z))\land \exists xP(x,u)\)}{\(\forall u\forall y(P(u,y)\vee Q(y,z))\land \exists uP(u,y)\)}{\(\forall x\forall u(P(x,u)\vee Q(u,z))\land \exists xP(x,y)\)}{\(\forall x\forall y(P(x,y)\vee Q(y,z))\land \exists xP(x,u)\)}
	...
	\end{tabular}
	\choicelow{\textcircled{2}\(\to\)\textcircled{3}}{\textcircled{4}\(\to\)\textcircled{5}}{\textcircled{3}\(\to\)\textcircled{4}}{\textcircled{1}\(\to\)\textcircled{2}}
	
	\item “没有不犯错误的人”的逻辑符号化为(\quad).
	
	设 \(H(x)\)：\(x\) 是人,\(P(x)\)：\(x\) 犯错误.
	\choicelow{\(\forall x(H(x)\to P(x))\)}{\(\exists x(H(x)\to P(x))\)}{\(\neg(\exists x(H(x)\to \neg P(x)))\)}{\(\neg(\exists x(H(x)\land \neg P(x)))\)}
	
	\item 下面蕴涵关系成立的是(\quad).
	\choicemid{\(\exists x\forall yA(x,y)\Rightarrow \forall y\exists xA(x,y)\)}{\(\forall xP(x)\land \forall xQ(x)\Rightarrow \forall x(P(x)\vee Q(x))\)}{\(\forall xP(x)\to \forall xQ(x)\Rightarrow \forall x(P(x)\to Q(x))\)}{\(\exists xP(x)\to \forall xQ(x)\Rightarrow \forall x(P(x)\to Q(x))\)}
	
	\item 给定公式 \(\exists xP(x)\to \forall xP(x)\),当 \(D=\{a,b\}\) 时,解释(\quad)使该公式真值为 \(0\).
	\choicelow{\(P(a)=1,\ P(b)=0\)}{\(P(a)=0,\ P(b)=1\)}{\(P(a)=1,\ P(b)=1\)}{\(P(a)=0,\ P(b)=0\)}
